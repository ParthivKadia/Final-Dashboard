...
import { updateStore } from "@/shared/services/storeService";
import { useAppStore } from "@/shared/stores/useAppStore";
import type { Store, CreateStoreBody, StoreTheme } from "@/shared/types/store";
...
function ReadonlyField({ value, muted }: { value: string; muted?: boolean }) {
  return (
    <div className="w-full px-3.5 py-2.5 rounded-xl text-sm break-all"
      style={{
...
export default function StoreProfile() {
  ...
  return (
    <div className="site-page site-page-padding">
      <PageMeta title="Store Profile | Storely Dashboard" description="Manage your store profile" />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          {/* <div>
            ...
            <p className="text-sm site-subtext">
              storely.co.in/{activeStore.username} · Created {formatDate(activeStore.createdAt)}
            </p>
          </div> */}
            ...
            <Section title="Store details" badge={isEditing ? "Editing" : undefined}>
              <div className="space-y-4">
                ...
                <Field label="Username (URL)">
                  <ReadonlyField value={`storely.co.in/${activeStore.username}`} muted />
                  {isEditing && (
                ...
                <Field label="Store Logo">
                  {isEditing ? (
                    <div className="space-y-2">
                      ...
                      {draft.logoUrl && (
                        <p className="text-[11px] site-text-muted break-all">
                          {draft.logoUrl}
                        </p>
                      )}
                    </div>
                ...
                <Field label="Store Banner">
                  {isEditing ? (
                    <div className="space-y-2">
                      ...
                      {draft.bannerUrl && (
                        <p className="text-[11px] site-text-muted break-all">{draft.bannerUrl}</p>
                      )}
                    </div>
                  ) : (
                    <>
            ...
            <div className="site-card p-5">
              ...
              <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--border-medium)" }}>
                ...
                <div className="px-3 pt-7 pb-3" style={{ backgroundColor: "var(--card-bg)" }}>
                  ...
                  <p className="text-xs site-text-muted site-truncate">storely.co.in/{activeStore.username}</p>
                  {cur.bio && (
